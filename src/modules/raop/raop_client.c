...
static ssize_t udp_send_audio_packet(pa_raop_client *c, pa_bool_t retrans, uint8_t *buffer, size_t size) {
    ssize_t length;
...
int pa_raop_client_udp_handle_control_packet(pa_raop_client *c, const uint8_t packet[], ssize_t size) {
    ...
    switch (playload) {
        ...
        default:
            pa_log_debug("Got an unexpected payload type on control channel: %u !", playload);
            return 1;
    }
