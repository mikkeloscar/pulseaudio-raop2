...
struct pa_raop_client {
    ...
    uint16_t my_control_port;
    uint16_t my_timing_port;
...
static void rtsp_cb(pa_rtsp_client *rtsp, pa_rtsp_state state, pa_headerlist *headers, void *userdata) {
    ...
    switch (state) {
        ...
        case STATE_ANNOUNCE: {
            ...
            pa_assert(c->control_fd < 0);
            pa_assert(c->timing_fd < 0);

            c->control_fd = open_bind_udp_socket(c, &c->my_control_port);
            if (c->control_fd < 0)
                goto error_announce;
            c->timing_fd  = open_bind_udp_socket(c, &c->my_timing_port);
            if (c->timing_fd < 0)
                goto error_announce;
        ...
        error_announce:
            ...
            c->my_control_port = c->server_control_port = DEFAULT_CONTROL_PORT;
            c->my_timing_port = c->server_timing_port = DEFAULT_TIMING_PORT;
        ...
        case STATE_SETUP: {
                    ...
                    if ((pc = strstr(token, "="))) {
                        *pc = 0;
                        if (pa_streq(token, "control_port")) {
                            port = 0;
                            pa_atou(pc + 1, &port);
                            c->server_control_port = port;
                        }
                        if (pa_streq(token, "timing_port")) {
                            port = 0;
                            pa_atou(pc + 1, &port);
                            c->server_timing_port = port;
                        }
                        *pc = '=';
                    }
                    pa_xfree(token);
            ...
            if (stream_port == 0)
                goto error;
            if (c->server_control_port == 0 || c->server_timing_port == 0)
                goto error;

            pa_log_debug("Using server_port=%d, control_port=%d & timing_port=%d",
                stream_port,
                c->server_control_port,
                c->server_timing_port);

            pa_assert(c->stream_fd < 0);
            pa_assert(c->control_fd >= 0);
            pa_assert(c->timing_fd >= 0);

            c->stream_fd = connect_udp_socket(c, -1, stream_port);
            if (c->stream_fd <= 0)
                goto error;
            ret = connect_udp_socket(c, c->control_fd, c->server_control_port);
            if (ret < 0)
                goto error;
            ret = connect_udp_socket(c, c->timing_fd, c->server_timing_port);
            if (ret < 0)
                goto error;
